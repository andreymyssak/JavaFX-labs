    ...
    private void solveTask() {
        ...
        try {
            ...
            for (int i = 0; i < coordinates.size(); i++) {
                ...
                try{
                    long numFromArrMultiply = arrMultiply.get(i);
                    int numFromArrSum = arrSum.get(i-1);

                    long subtractionNumbers = numFromArrMultiply - numFromArrSum;

                    if (subtractionNumbers < 0) {
                        coordinates.get(i).setValue2(String.valueOf("-"));
                    } else {
                        int valueI = arrCoordinates.get(i);
                        int valueIMinusOne = arrCoordinates.get(i-1);

                        double solutionSin = Math.pow(Math.sin(valueI + valueIMinusOne), 2);
                        if (solutionSin == 0) {
                            coordinates.get(i).setValue2(String.valueOf("-"));
                        } else {
                            int absValueI = Math.abs(valueI);
                            double solution = (Math.sqrt(subtractionNumbers/solutionSin))/absValueI;
